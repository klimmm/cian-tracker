...
TAG_STYLES = {
    ...
    "price_down": {"backgroundColor": "#e6f7f5", "color": "#2a9d8f"},
    "price_up": {"backgroundColor": "#fbe9e7", "color": "#d62828"},
    "good_price": {"backgroundColor": "#fcf3cd", "color": "#856404"},
    "cian_estimate": {
        "backgroundColor": "#f5f5f5",
        "color": "#555555",
        "fontSize": "0.9rem",
    },
    "days_new": {"backgroundColor": "#e8f5e9", "color": "#2e7d32"},
    "days_recent": {"backgroundColor": "#e3f2fd", "color": "#1565c0"},
    "days_medium": {"backgroundColor": "#fff3e0", "color": "#e65100"},
    "days_old": {"backgroundColor": "#ffebee", "color": "#c62828"},
    "days_inactive": {"backgroundColor": "#f0f0f0", "color": "#707070"},
    "walk_close": {"backgroundColor": "#4285f4", "color": "#ffffff"},
    "walk_medium": {"backgroundColor": "#aecbfa", "color": "#174ea6"},
    "walk_far": {"backgroundColor": "#dadce0", "color": "#3c4043"},
    "neighborhood_hamovniki": {"backgroundColor": "#e0f7f7", "color": "#0c5460"},
    "neighborhood_arbat": {"backgroundColor": "#d0d1ff", "color": "#3f3fa3"},
    "neighborhood_other": {"backgroundColor": "#dadce0", "color": "#3c4043"},
}


class TagCreator:
    """Unified tag creation system."""
    ...
    def create_tag(text, tag_type="default", custom_styles=None):
        ...
        # Build style string
        style_str = "; ".join(
            [f"{StyleUtils.camel_to_kebab(k)}:{v}" for k, v in base_styles.items()]
        )
...
class ColumnFormatter:
    """Centralized column formatting for consistent display."""
    ...
    def format_update_title(row):
        ...
        if pd.notnull(days_active) and days_active != "--":
            tags.append(
                TagFormatter.format_days_active_tag(
    ...
    def format_property_tags(row):
        ...
        if neighborhood := tag_flags.get("neighborhood"):
            tags.append(
                TagFormatter.format_neighborhood_tag(
                    neighborhood,
    ...
    def format_activity_date(row):
        ...
        if pd.notnull(row.get("updated_time_sort")) and pd.notnull(
            row.get("activity_date_sort")
        ):
            time_diff = abs(
    ...
    def format_active_days(row):
        ...
        days_value = row.get("days_active_value", 0)
        tag = TagFormatter.format_days_active_tag(
            days_value, row.get("status", "active")
        )
...
class DataManager:
    ...
    def _create_display_columns(df):
        ...
        if all(col in df.columns for col in ["update_title", "activity_date"]):
    ...
    def generate_tags_for_row(row):
        ...
        try:
            if "updated_time_sort" in row and not pd.isna(row["updated_time_sort"]):
                recent_time = pd.Timestamp.now(MOSCOW_TZ) - pd.Timedelta(hours=24)
                if row["updated_time_sort"] > recent_time:
