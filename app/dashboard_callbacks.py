...
def register_data_callbacks(app):
    ...
    @app.callback(
        [Output("apartment-data-store", "data"), Output("last-update-time", "children")],
    ...
    def load_apartment_data(_):
        ...
        try:
            ...
            required_cols = CONFIG["columns"]["display"] + ["details", "offer_id", "date_sort_combined"]
...
def register_details_callbacks(app):
    ...
    def handle_apartment_panel(active_cell, prev_clicks, next_clicks, close_clicks, 
        ...
        visible_style = StyleManager.merge_styles(current_style, {
            "opacity": "1", "visibility": "visible", "pointer-events": "auto", "display": "block"
        })
        ...
        if (triggered_id == "apartment-table" and active_cell and 
            active_cell.get("column_id") == "details"):
            return handle_apartment_selection(active_cell["row"], table_data, visible_style)

        # Handle navigation
        if triggered_id in ["prev-apartment-button", "next-apartment-button"] and selected_data:
            return handle_apartment_navigation(triggered_id, selected_data, visible_style)
...
def handle_button_click(trigger_id, current_filters):
    ...
    if trigger_id in [btn["id"] for btn in DISTANCE_BUTTONS]:
    ...
    if trigger_id in filter_map:
        filter_key = filter_map[trigger_id]
        current_filters[filter_key] = not current_filters.get(filter_key, False)
        return True
        
    # Sort buttons
    if trigger_id in [btn["id"] for btn in SORT_BUTTONS]:
        button = next((btn for btn in SORT_BUTTONS if btn["id"] == trigger_id), None)
        if button:
            ...
            if current_filters.get("active_sort_btn") == trigger_id:
                ...
                current_filters["sort_direction"] = button.get("default_direction", "asc")
            return True
...
def create_data_table(df):
    ...
    numeric_cols = {"distance", "price_value_formatted", "cian_estimation_formatted", 
                    "price_difference_formatted", "monthly_burden_formatted"}
    markdown_cols = {"price_change_formatted", "address_title", "offer_link", "price_info", 
    ...
    markdown_style = [{"if": {"column_id": col}, "textAlign": "center"} for col in markdown_cols]
    ...
    return dash_table.DataTable(
        ...
        style_table={"overflowX": "auto", "maxWidth": "1200px", "width": "100%", "margin": "0 auto"},
        ...
        style_data_conditional=COLUMN_STYLES + [
            {"if": {"column_id": "details"}, "fontWeight": "normal", "cursor": "pointer !important"}
        ],
        page_size=100,
        page_action="native",
