...
class DataFilterSorter:
    ...
    def apply_sorting(df):
        ...
        logger.debug(f"apply_sorting called with dataframe of shape: {df.shape if not df.empty else 'EMPTY'}")
        ...
        if "status" in df.columns:
            logger.debug(f"Creating sort key based on status column, unique values: {df['status'].unique()}")
            ...
            if "distance_sort" in df.columns:
                logger.debug("Sorting by status and distance")
                df = df.sort_values(["sort_key", "distance_sort"], ascending=[True, True])
            else:
                logger.debug("Sorting by status only (distance_sort not available)")
        ...
        logger.debug(f"apply_sorting returning dataframe of shape: {df.shape}")
        return df

    @staticmethod
    def filter_and_sort_data(df, filters=None, sort_by=None):
        """Filter and sort data based on provided criteria."""
        logger.debug(f"filter_and_sort_data called with:")
        logger.debug(f"  - dataframe shape: {df.shape if not df.empty else 'EMPTY'}")
        ...
        # Add debug debugrmation about distance values
        if "distance_sort" in df.columns:
            logger.debug(f"Distance stats: min={df['distance_sort'].min()}, max={df['distance_sort'].max()}, mean={df['distance_sort'].mean():.2f}")
            ...
            logger.debug(f"Sample of {sample_size} unique distance values: {unique_distances[:sample_size]}")
            
            # Also show value counts for better understanding
            value_counts = df['distance_sort'].value_counts().sort_index()
            logger.debug(f"Distance value distribution (first 10 entries):\n{value_counts.head(10)}")
        else:
        ...
        if "distance" in df.columns:
            logger.debug(f"Human-readable distance sample (first 10): {df['distance'].head(10).tolist()}")
        ...
        if filters:
            ...
            if price_value := filters.get("price_value"):
                if price_value != float("inf") and "price_value" in df.columns:
                    logger.debug(f"Applying price filter: <= {price_value}")
                    df = df[df["price_value"] <= price_value]
                    logger.debug(f"After price filter: {len(df)}/{df_size_before} rows remaining")
                else:
            ...
            if distance_value := filters.get("distance_value"):
                if distance_value != float("inf") and "distance_sort" in df.columns:
                    logger.debug(f"Applying distance filter: <= {distance_value}")
                    # Log the number of rows that would pass the filter
                    passing_rows = (df["distance_sort"] <= distance_value).sum()
                    logger.debug(f"Only {passing_rows} rows would pass distance filter <= {distance_value}")
                    df = df[df["distance_sort"] <= distance_value]
                    logger.debug(f"After distance filter: {len(df)}/{df_size_before} rows remaining")
                else:
        ...
        if filters:
            ...
            if filters.get("nearest") and "distance_sort" in df.columns:
                logger.debug("Applying nearest filter: < 1.5")
                df = df[df["distance_sort"] < 1.5]
                logger.debug(f"After nearest filter: {len(df)}/{df_size_before} rows remaining")
            ...
            if filters.get("below_estimate") and "price_difference_value" in df.columns:
                logger.debug("Applying below_estimate filter: >= 5000")
                df = df[df["price_difference_value"] >= 5000]
                logger.debug(f"After below_estimate filter: {len(df)}/{df_size_before} rows remaining")
            ...
            if filters.get("inactive") and "status" in df.columns:
                logger.debug("Applying inactive filter: status == active")
                df = df[df["status"] == "active"]
                logger.debug(f"After inactive filter: {len(df)}/{df_size_before} rows remaining")
            ...
            if filters.get("updated_today") and "updated_time_sort" in df.columns:
                logger.debug("Applying updated_today filter")
                try:
                    ...
                    logger.debug(f"After updated_today filter: {len(df)}/{df_size_before} rows remaining")
            ...
            if "sort_column" in filters and "sort_direction" in filters:
                ...
                if sort_column in df.columns:
                    logger.debug(f"Sorting by filter column: {sort_column} ({filters['sort_direction']})")
                    df = df.sort_values(
        ...
        # Apply sorting from sort_by parameter
            ...
            for item in sort_by:
                ...
                if col in df.columns:
                    logger.debug(f"Sorting by sort_by column: {col} ({item['direction']})")
                    df = df.sort_values(col, ascending=item["direction"] == "asc")
                else:
                    logger.warning(f"Cannot sort by {col}: column not in dataframe")

        logger.debug(f"filter_and_sort_data returning dataframe of shape: {df.shape} ({len(df)}/{df_size_before} rows)")
        return df